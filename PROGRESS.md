...
- 2026-07-14: implemented `FINOS-MCP-01` on `codex/finos-vision-mcp`; Zebra keeps the configured text model as the planning Agent and conditionally advertises only `mcp.minimax.understand_image` through the existing MCP proxy route. The MiniMax adapter accepts only task-local JPEG, PNG, or WebP files up to 20 MB, rejects remote and escaped paths before egress, returns text as untrusted tool evidence, and exposes source and provider metadata in the normal trace. The synchronous API path used by FinOS and the durable worker path both wire the tool only when explicitly enabled, with exact-tool read-only preapproval and no broad MCP permission. Synchronous responses now include aggregate model-token and tool-call usage. All `999` tests, Ruff, Mypy across `219` source files, and the 8-case eval release gate passed; deployment and one real FinOS screenshot acceptance remain open.
