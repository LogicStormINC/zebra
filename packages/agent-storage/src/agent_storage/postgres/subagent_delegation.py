...
class PostgresSubagentDelegationStore:
    ...
    def delegate(
        ...
        """Materialize the child atomically with its delegation link."""
        ...
        receipt = self._admission.admit(child_admission)
        ...
        if inserted is None:
            existing = self._find_by_key(request)
            ...
            return SubagentDelegationReceipt(
                ...
                idempotency_key=request.idempotency_key,
                child_task_id=TaskId(UUID(str(existing.child_task_id))),
                child_binding_digest=existing.child_binding_digest or "0" * 64,
                status="replayed",
            )
        return SubagentDelegationReceipt(
            delegation_id=delegation_id,
