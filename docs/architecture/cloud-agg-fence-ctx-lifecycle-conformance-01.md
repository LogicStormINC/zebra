...
| `commit_administrative_activation` | `AdministrativeMutationCAS`，无 Worker LeaseFence | authority namespace/session 与 Event 一致；Capsule 读取受 Store namespace 约束并校验同 Session | `require_administrative_projections` 锁 Session stream，检查 Session/Workspace 与 expected revision；active pointer `FOR UPDATE`；Event sequence CAS | Event、pointer、Session/Workspace 在一个连接事务；pointer 使用 expected capsule CAS，失败整体回滚 | `test_administrative_context_missing_pointer_rolls_back_event`、namespace/projection drift、API recovery `19/19` 记录 | **`BLOCK-GAP`：Store 未校验 `event.event_type == CONTEXT_COMPACTED`，也未校验 recovery payload 的 `capsule_id` 绑定。直接 Port 调用可绕过当前 API caller 约束。** |
...
`commit_administrative_activation` 的事务边界已经正确锁定 stream、校验
projection、执行 pointer CAS，并能在失败时回滚；但它把 Event 语义完整性留给
caller。当前 API caller 固定生成 `CONTEXT_COMPACTED`，这不足以证明 Port 本身
不能被其他 composition 或直接 adapter 调用绕过。一个错误 Event type 或与
`capsule_id` 不一致的 recovery payload 仍可能通过同一 Administrative CAS 提交
语义不一致的激活记录。

因此本卡不得标记 `Done`，也不得激活 `CLOUD-AGG-FENCE-01`。最小后续实现卡为：

`CLOUD-AGG-FENCE-CTX-SEMANTIC-01`（`Ready`）

- Owned paths：`packages/agent-storage/src/agent_storage/postgres/context_lifecycle.py`、
  对应 `tests/agent_storage/` 与 `tests/api/` focused tests，以及该卡治理记录；
...
- 验收：错误 Event type、缺失/错误 capsule binding、错误 namespace、旧 stream
  ...
  真实 PostgreSQL focused matrix 通过。
...
- `CLOUD-AGG-FENCE-CTX-LIFECYCLE-CON-01` 保持 `In Progress / BLOCK-GAP`，其
  successor 只修复上述最小语义缺口。
- 在缺口修复并通过真实 PostgreSQL 证据前，禁止修改 `apps/api/`、
  `apps/worker/` 启动与 profile selection、`agent-runtime`、Provider HTTP、
  SQLite、Redis、Mem0、CopilotKit/Trench 或 application Compose。
