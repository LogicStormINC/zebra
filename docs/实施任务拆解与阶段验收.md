...
> Phase 0-8 is complete. This document preserves the original dependency order
> and acceptance model; it is not the current task queue. Use `PROGRESS.md` for
> the mainline snapshot and `docs/AGENT_TASKS.md` for executable work.
...
本文档最初优先解决 Phase 0 到 Phase 4 的落地问题，并为 Phase 5-8
保留扩展接口。该基线现已完成；后续增量不再通过继续增加 Phase 编号表达优先级。

### 1.1 产品定位基线

- Zebra Agent 的目标是通用执行型 Agent Runtime 与工作台，不是代码生成产品。
- Claude Code 和 Codex 是 Harness、工具执行、会话恢复与 HITL 交互的参考，不是任务域限制。
- 代码、Git 和 SCM 能力保留为可选工具；默认前端只呈现通用任务执行与结果。
- 人工操作区必须由真实 approval 驱动，无 approval 时不得显示 Commit、PR 等休眠控件。
...
- Python 实现
- 本地工作目录
- 受控工具集合；本地阶段可包含 shell/file/patch/test/git 适配器
...
```text
当前执行规则：
1. 从 PROGRESS.md 确认主线状态、能力边界和待决策项
2. 从 docs/AGENT_TASKS.md 选择一个 Ready 任务
3. 按任务卡声明 Owner、Branch、Owned paths 和 Acceptance
4. Locked 任务必须等待显式维护者决策，不因本文档中的目标架构自动解锁
```

Phase 0-8 的顺序仍用于解释依赖关系、回归测试和历史验收，不再表示
“本周计划”或当前开发阶段。
