...
`CLOUD-REC-RESTORE-01` 已作为下一主线子任务激活，其余恢复子任务保持未激活。
...
- 首次容器验证发现生产镜像以 `--no-dev` 同步时缺少 `uvicorn`；现已将其声明为
  ...
  Application Compose 现默认使用 helper 确认的 Python mirror，完整 runner 已返回
  `ZEBRA_APPLICATION_COMPOSE_TEST_RESULT=PASS`；Dockerfile 单独构建仍保留官方
  digest 默认值，直接访问 Docker Hub 的证据缺口不改变提交 pin。
...
`CLOUD-REC-BACKUP-01` 已 `Done`，`CLOUD-REC-RESTORE-01` 正在推进，其逻辑备份/恢复
证据不构成生产 PITR/RPO/RTO 声明；
