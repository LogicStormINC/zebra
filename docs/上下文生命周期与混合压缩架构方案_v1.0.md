...
本文是专项架构约束，不代表当前主线已经实现其中所有能力。实施前必须在 `docs/AGENT_TASKS.md` 创建并认领任务卡。
...
Phase 117 的目标是“有界、确定性的会话压缩”。其显式非目标包括语义摘要、provider tokenizer 保证和 provider-native compaction，因此不能把当前实现描述成本文的完整目标架构。
...
1. Context Compiler 和 conversation compactor 仍使用固定预算，不能反映不同模型的真实窗口、最大输出、reasoning 和工具 schema 成本。
2. token 估算主要基于字符近似，不是 provider-aware tokenizer 或 count-tokens 结果。
3. 初始模型请求没有经过与后续 tool loop 完全一致的 conversation compaction 路径。
4. `within_budget=false` 没有形成统一的发送前硬阻断。
5. 当前压缩摘要主要通过固定字段和字符截断生成，缺少版本化、可恢复的语义 Capsule。
6. `ContextCompacted` 事件记录指标，但不保存可独立恢复的 Capsule 内容和来源覆盖证明。
7. 通用命令 stdout/stderr 缺少统一的 artifact 化输出信封，单次长日志可能污染窗口。
8. greedy whole-item selection 可能跳过体积较大但关键的规则或证据。
9. provider-native compaction 尚未抽象为可选能力，跨 provider continuation 也没有正式契约。
10. 当前没有 `/context`、`/compact [focus]`、压缩预览或从历史 Capsule 恢复的操作入口。
...
### CTX-LC-02：渐进式 Active Projection
...
### CTX-LC-03：Durable Context Capsule
...
### CTX-LC-04：Provider-native Adapters

- OpenAI Responses compaction；
- Anthropic compaction 与 context editing；
...
任务必须按上述依赖顺序认领。不能先接入 opaque provider state，再补 Zebra 自己的 Capsule 和恢复边界。
