...
- 模型调用额度只限制请求次数，不预先削弱最后一次允许请求的能力；最后一次
  模型请求仍可调用工具。如果工具执行后还需要新的模型轮次，才以
  `model_call_budget_exhausted` 挂起。
...
- 普通交互 Host 必须省略模型/工具硬额度，继承核心的无次数上限默认值；
  不得在适配器里重新注入低上限。
- 定时任务、批处理和管理员运行可以保留明确展示、可配置、可审计的硬额度。
- 预算挂起必须被 AG-UI 投影为带稳定 `reason` 的 terminal interrupt，确保
  SSE 客户端结束等待并显示可恢复状态；`waiting_children` 等内部挂起不应
  被误投影成用户输入中断。
- 禁用工具的收尾提示不得让模型向用户暴露内部“额度已用完”措辞；证据不足时
  只陈述尚缺少的事实。
