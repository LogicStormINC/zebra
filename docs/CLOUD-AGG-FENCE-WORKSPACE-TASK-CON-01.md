...
| WT-11 | Direct Task mutation authority | **BLOCK-GAP** | `AgentTaskPort.attach_segment` 和 `PostgresAgentTaskStore.attach_segment` 只接收 task/segment identity，不接收 `WorkerMutationAuthority`，也不直接校验当前 LeaseFence；若被 Worker 直接调用可绕过 authority。 |
...
结论为 `BLOCK-GAP`，任务进入 `Review`。Workspace Worker transaction 和 Handoff
组合路径已有完整的局部证据；但 Task direct mutation 尚未在 Port/adapter 边界
表达 Worker authority，且 Workspace/Task PostgreSQL 证据缺少当前仓库内可重放的
runner。缺口不是应用 Compose、Runtime selector 或 SQLite fallback 的授权理由。

## 必须单独登记的后续卡

1. `CLOUD-AGG-FENCE-TASK-01`（`Locked`）：为 Worker-owned Task rollover 增加
   明确的 authority/fence 入口，或将 direct adapter 明确收窄为 management/Handoff
   transaction-only，补齐 namespace、owner、epoch、token、stale CAS 和 zero-write
   回归；保持已有 Handoff helper 的 Owned path 不被重做。
