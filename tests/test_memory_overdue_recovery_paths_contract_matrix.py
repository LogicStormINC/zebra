...
def test_memory_overdue_recovery_paths_contract_matrix_matches_api_and_cli(
    tmp_path: Path,
) -> None:
    ...
    SQLiteMemoryStore(api_database_path).upsert(
        _memory_record(
            memory_id="00000000-0000-0000-0000-000000001011",
    ...
    SQLiteMemoryStore(cli_database_path).upsert(
        _memory_record(
            memory_id="00000000-0000-0000-0000-000000001012",
    ...
    http_response = TestClient(create_http_app(api_database_path)).post(
        f"/sessions/{api_session_id}/memory-overdue-recovery-paths",
...
def _normalize_http(payload: dict[str, object]) -> dict[str, object]:
    return {
        ...
        "overdue_recovery_path_counts": payload["overdue_recovery_path_counts"],
        "highest_priority_overdue_recovery_path": payload[
            "highest_priority_overdue_recovery_path"
        ],
        "highest_priority_overdue_recovery_priority": payload[
            "highest_priority_overdue_recovery_priority"
        ],
        "highest_priority_overdue_recovery_scope_kind": payload[
...
def _seed_completed_session(database_path: Path, workspace_root: Path) -> SessionId:
    bootstrap = SessionBootstrapService().build(
        SessionBootstrapCommand(
            title="Memory overdue recovery paths matrix",
