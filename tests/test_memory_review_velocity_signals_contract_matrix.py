...
def test_memory_review_velocity_signals_contract_matrix_matches_across_api_and_cli(
    tmp_path: Path,
) -> None:
    ...
    api_reviewed = _memory_record(
        memory_id="00000000-0000-0000-0000-000000000371",
    ...
    cli_reviewed = _memory_record(
        memory_id="00000000-0000-0000-0000-000000000372",
    ...
    http_response = TestClient(create_http_app(api_database_path)).post(
        f"/sessions/{api_session_id}/memory-velocity",
...
def _normalize_http(payload: dict[str, object]) -> dict[str, object]:
    return {
        ...
        "total_reviewed_last_7d_count": payload["total_reviewed_last_7d_count"],
        "total_reviewed_last_30d_count": payload["total_reviewed_last_30d_count"],
        "latest_review_scope_kind": payload["latest_review_scope_kind"],
        "latest_review_window": payload["latest_review_window"],
    }
...
def _seed_completed_session(database_path: Path, workspace_root: Path) -> SessionId:
    bootstrap = SessionBootstrapService().build(
        SessionBootstrapCommand(
            title="Memory velocity signals matrix",
