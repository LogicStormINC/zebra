...
def test_follow_through_verification_states_contract_matrix_matches_api_and_cli(
    tmp_path: Path,
) -> None:
...
def _contract_summary(payload: dict[str, object]) -> dict[str, object]:
    return {
        ...
        "overdue_retention_breach_follow_through_verification_counts": payload[
        ...
        "highest_priority_overdue_retention_breach_follow_through_verification_state": (
            payload["highest_priority_overdue_retention_breach_follow_through_verification_state"]
        ),
        "highest_priority_overdue_retention_breach_follow_through_verification_scope_kind": (
            payload["highest_priority_overdue_retention_breach_follow_through_verification_scope_kind"]
        ),
        "highest_priority_overdue_retention_breach_follow_through_verification_memory_id": (
            payload["highest_priority_overdue_retention_breach_follow_through_verification_memory_id"]
        ),
...
def _seed_completed_session(database_path: Path, workspace_root: Path) -> SessionId:
    bootstrap = SessionBootstrapService().build(
        SessionBootstrapCommand(
            title="Memory overdue retention breach follow-through verification states matrix",
            user_input="Inspect overdue retention breach follow-through verification states.",
