...
def test_user_memory_bulk_review_contract_matrix_matches_across_api_and_cli(
    tmp_path: Path,
) -> None:
    ...
    api_record = _candidate_record("00000000-0000-0000-0000-000000000221", api_session_id)
    cli_record = _candidate_record("00000000-0000-0000-0000-000000000222", cli_session_id)
    ...
    http_response = TestClient(create_http_app(api_database_path)).post(
        "/users/user-1/memory/bulk-review",
        json={
            "decision": "confirm",
            "memory_ids": [str(api_record.memory_id), "not-a-uuid"],
        },
    )
    cli_result = execute(
        [
...
def _seed_completed_session(database_path: Path, workspace_root: Path) -> SessionId:
    bootstrap = SessionBootstrapService().build(
        SessionBootstrapCommand(
            title="Memory scope bulk review matrix",
...
def _candidate_record(memory_id: str, session_id: SessionId) -> MemoryRecord:
    created_at = datetime(2026, 7, 5, 9, 0, tzinfo=UTC)
    return MemoryRecord(
