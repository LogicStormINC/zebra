...
def test_memory_overdue_trend_signals_contract_matrix_matches_across_api_and_cli(
    tmp_path: Path,
) -> None:
    ...
    SQLiteMemoryStore(api_database_path).upsert(
        _memory_record(
            memory_id="00000000-0000-0000-0000-000000000921",
    ...
    SQLiteMemoryStore(cli_database_path).upsert(
        _memory_record(
            memory_id="00000000-0000-0000-0000-000000000922",
    ...
    http_response = TestClient(create_http_app(api_database_path)).post(
        f"/sessions/{api_session_id}/memory-overdue-trends",
        json={"as_of": "2026-07-10T00:00:00+00:00"},
    )
    cli_result = execute(
        [
...
def _normalize_http(payload: dict[str, object]) -> dict[str, object]:
    return {
        ...
        "overdue_trend_signal_counts": payload["overdue_trend_signal_counts"],
        "highest_priority_overdue_trend_signal": payload[
            "highest_priority_overdue_trend_signal"
        ],
        "highest_priority_overdue_trend_rank": payload[
            "highest_priority_overdue_trend_rank"
        ],
        "highest_priority_overdue_trend_scope_kind": payload[
...
def _seed_completed_session(database_path: Path, workspace_root: Path) -> SessionId:
    bootstrap = SessionBootstrapService().build(
        SessionBootstrapCommand(
            title="Memory overdue trend signals matrix",
