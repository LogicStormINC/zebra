...
def test_session_memory_review_contract_matrix_confirm_matches_across_api_and_cli(
    tmp_path: Path,
) -> None:
    ...
    api_record = _candidate_record(api_session_id, str(api_workspace.resolve()))
    cli_record = _candidate_record(cli_session_id, str(cli_workspace.resolve()))
    ...
    http_response = TestClient(create_http_app(api_database_path)).post(
        f"/sessions/{api_session_id}/memory/{api_record.memory_id}/confirm",
        json={},
    )
    cli_result = execute(
        [
...
def test_session_memory_review_contract_matrix_invalid_state_matches_across_api_and_cli(
    tmp_path: Path,
) -> None:
    api_database_path = tmp_path / "api.sqlite"
    cli_database_path = tmp_path / "cli.sqlite"
    ...
    api_session_id = _seed_completed_session(api_database_path, api_workspace)
    cli_session_id = _seed_completed_session(cli_database_path, cli_workspace)
    api_record = _candidate_record(api_session_id, str(api_workspace.resolve())).model_copy(
        update={"status": MemoryStatus.CONFIRMED}
    )
    cli_record = _candidate_record(cli_session_id, str(cli_workspace.resolve())).model_copy(
        update={"status": MemoryStatus.CONFIRMED}
    )
    SQLiteMemoryStore(api_database_path).upsert(api_record)
    SQLiteMemoryStore(cli_database_path).upsert(cli_record)
    ...
    http_response = TestClient(create_http_app(api_database_path)).post(
        f"/sessions/{api_session_id}/memory/{api_record.memory_id}/expire",
        json={},
    )
    cli_result = execute(
        [
...
def _normalize_http(payload: dict[str, object]) -> dict[str, object]:
    if payload.get("status") == "invalid_state":
        return {
...
def _candidate_record(session_id: SessionId, repo_id: str) -> MemoryRecord:
    created_at = datetime(2026, 7, 2, 11, 0, tzinfo=UTC)
    return MemoryRecord(
        memory_id=MemoryId(UUID("00000000-0000-0000-0000-000000000133")),
