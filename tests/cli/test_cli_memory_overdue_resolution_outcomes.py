...
def test_cli_memory_overdue_resolution_outcomes_classify_pending_next_review_confirmation(
    tmp_path: Path,
) -> None:
    ...
    SQLiteMemoryStore(database_path).upsert(
        _memory_record(
            memory_id="00000000-0000-0000-0000-000000001061",
    ...
    assert result.command == "memory-overdue-resolution-outcomes"
    assert result.payload["overdue_resolution_outcome_counts"] == {
        "pending_next_review_confirmation": 1
    }
    assert result.payload["highest_priority_overdue_resolution_outcome"] == (
        "pending_next_review_confirmation"
    )
    assert result.payload["highest_priority_overdue_resolution_outcome_scope_kind"] == "repo"


def _seed_completed_session(database_path: Path, workspace_root: Path) -> SessionId:
    bootstrap = SessionBootstrapService().build(
        SessionBootstrapCommand(
            title="CLI memory overdue resolution outcomes",
