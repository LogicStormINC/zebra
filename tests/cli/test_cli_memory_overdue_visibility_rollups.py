...
def test_cli_memory_overdue_visibility_rollups_aggregate_scope_visibility_counts(
    tmp_path: Path,
) -> None:
    ...
    store.upsert(
        _memory_record(
            memory_id="00000000-0000-0000-0000-000000000881",
    ...
    store.upsert(
        _memory_record(
            memory_id="00000000-0000-0000-0000-000000000882",
            session_id=session_id,
            visibility=MemoryVisibility.REPO,
            memory_type=MemoryType.PROCEDURE,
    ...
    store.upsert(
        _memory_record(
            memory_id="00000000-0000-0000-0000-000000000883",
            session_id=session_id,
    ...
    assert result.command == "memory-overdue-visibility"
    assert result.payload["overdue_scope_count"] == 1
    assert result.payload["overdue_memory_visibility_counts"] == {"repo": 2}
    assert result.payload["highest_priority_overdue_memory_visibility"] == "repo"
    assert result.payload["highest_priority_overdue_visibility_scope_kind"] == "repo"


def _seed_completed_session(database_path: Path, workspace_root: Path) -> SessionId:
    bootstrap = SessionBootstrapService().build(
        SessionBootstrapCommand(
            title="CLI memory overdue visibility rollups",
