...
def test_cli_memory_overdue_retention_breach_owner_targets_classify_retention_admin(
    tmp_path: Path,
) -> None:
    ...
    assert result.command == "memory-overdue-retention-breach-owner-targets"
    assert result.payload["overdue_retention_breach_owner_target_counts"] == {
        "retention_admin": 1
    }
    assert result.payload["highest_priority_overdue_retention_breach_owner_target"] == (
        "retention_admin"
    )
    assert (
        result.payload["highest_priority_overdue_retention_breach_owner_target_scope_kind"]
        == "repo"
    )
...
def _seed_completed_session(database_path: Path, workspace_root: Path) -> SessionId:
    bootstrap = SessionBootstrapService().build(
        SessionBootstrapCommand(
            title="CLI memory overdue retention breach owner targets",
