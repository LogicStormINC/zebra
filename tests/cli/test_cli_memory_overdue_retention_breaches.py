...
def test_cli_memory_overdue_retention_breaches_classify_weekly_breach(
    tmp_path: Path,
) -> None:
    ...
    assert result.command == "memory-overdue-retention-breaches"
    assert result.payload["overdue_retention_breach_counts"] == {
        "extended_window_breached": 1
    }
    assert result.payload["highest_priority_overdue_retention_breach"] == (
        "extended_window_breached"
    )
    assert result.payload["highest_priority_overdue_retention_breach_scope_kind"] == "repo"


def _seed_completed_session(database_path: Path, workspace_root: Path) -> SessionId:
    bootstrap = SessionBootstrapService().build(
        SessionBootstrapCommand(
            title="CLI memory overdue retention breaches",
