...
def test_cli_memory_overdue_retention_breach_lanes_classify_emergency_lane(
    tmp_path: Path,
) -> None:
    ...
    assert result.command == "memory-overdue-retention-breach-lanes"
    assert result.payload["overdue_retention_breach_lane_counts"] == {
        "emergency_retention_override_lane": 1
    }
    assert result.payload["highest_priority_overdue_retention_breach_lane"] == (
        "emergency_retention_override_lane"
    )
    assert result.payload["highest_priority_overdue_retention_breach_lane_scope_kind"] == "repo"


def _seed_completed_session(database_path: Path, workspace_root: Path) -> SessionId:
    bootstrap = SessionBootstrapService().build(
        SessionBootstrapCommand(
            title="CLI memory overdue retention breach lanes",
