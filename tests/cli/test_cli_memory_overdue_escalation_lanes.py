...
def test_cli_memory_overdue_escalation_lanes_classify_local_queue_lane(
    tmp_path: Path,
) -> None:
    ...
    SQLiteMemoryStore(database_path).upsert(
        _memory_record(
            memory_id="00000000-0000-0000-0000-000000000971",
    ...
    assert result.command == "memory-overdue-escalation-lanes"
    assert result.payload["overdue_escalation_lane_counts"] == {"local_queue_lane": 1}
    assert result.payload["highest_priority_overdue_escalation_lane"] == "local_queue_lane"
    assert result.payload["highest_priority_overdue_escalation_scope_kind"] == "repo"


def _seed_completed_session(database_path: Path, workspace_root: Path) -> SessionId:
    bootstrap = SessionBootstrapService().build(
        SessionBootstrapCommand(
            title="CLI memory overdue escalation lanes",
