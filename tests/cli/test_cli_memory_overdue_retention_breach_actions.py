...
def test_cli_memory_overdue_retention_breach_actions_classify_force_archive(
    tmp_path: Path,
) -> None:
    ...
    assert result.command == "memory-overdue-retention-breach-actions"
    assert result.payload["overdue_retention_breach_action_counts"] == {
        "force_archive_or_override": 1
    }
    assert result.payload["highest_priority_overdue_retention_breach_action"] == (
        "force_archive_or_override"
    )
    assert result.payload["highest_priority_overdue_retention_breach_action_scope_kind"] == (
        "repo"
    )
...
def _seed_completed_session(database_path: Path, workspace_root: Path) -> SessionId:
    bootstrap = SessionBootstrapService().build(
        SessionBootstrapCommand(
            title="CLI memory overdue retention breach actions",
