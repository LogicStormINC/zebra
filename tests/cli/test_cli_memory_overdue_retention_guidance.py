...
def test_cli_memory_overdue_retention_guidance_classify_next_review_retention(
    tmp_path: Path,
) -> None:
    ...
    assert result.command == "memory-overdue-retention-guidance"
    assert result.payload["overdue_retention_guidance_counts"] == {
        "retain_until_next_review": 1
    }
    assert result.payload["highest_priority_overdue_retention_guidance"] == (
        "retain_until_next_review"
    )
    assert result.payload["highest_priority_overdue_retention_bucket"] == "standard"
    assert result.payload["highest_priority_overdue_retention_scope_kind"] == "repo"


def _seed_completed_session(database_path: Path, workspace_root: Path) -> SessionId:
    bootstrap = SessionBootstrapService().build(
        SessionBootstrapCommand(
            title="CLI memory overdue retention guidance",
