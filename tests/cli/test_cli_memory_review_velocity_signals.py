...
def test_cli_memory_review_velocity_signals_report_recent_review_windows(
    tmp_path: Path,
) -> None:
    ...
    reviewed_recent = _memory_record(
        memory_id="00000000-0000-0000-0000-000000000361",
    ...
    reviewed_week = _memory_record(
        memory_id="00000000-0000-0000-0000-000000000362",
        session_id=session_id,
    ...
    assert result.command == "memory-velocity"
    assert result.payload["scope_count"] == 2
    assert result.payload["total_reviewed_count"] == 2
    assert result.payload["total_reviewed_last_24h_count"] == 1
    assert result.payload["total_reviewed_last_7d_count"] == 2
    assert result.payload["latest_review_scope_kind"] == "repo"
    assert result.payload["latest_review_window"] == "last_24h"


def _seed_completed_session(database_path: Path, workspace_root: Path) -> SessionId:
    bootstrap = SessionBootstrapService().build(
        SessionBootstrapCommand(
            title="CLI memory velocity signals",
