...
def test_cli_memory_overdue_trend_signals_classify_emerging_overdue_scope(
    tmp_path: Path,
) -> None:
    ...
    SQLiteMemoryStore(database_path).upsert(
        _memory_record(
            memory_id="00000000-0000-0000-0000-000000000911",
    ...
    assert result.command == "memory-overdue-trends"
    assert result.payload["overdue_trend_signal_counts"] == {"emerging_overdue": 1}
    assert result.payload["highest_priority_overdue_trend_signal"] == "emerging_overdue"
    assert result.payload["highest_priority_overdue_trend_scope_kind"] == "repo"


def _seed_completed_session(database_path: Path, workspace_root: Path) -> SessionId:
    bootstrap = SessionBootstrapService().build(
        SessionBootstrapCommand(
            title="CLI memory overdue trend signals",
