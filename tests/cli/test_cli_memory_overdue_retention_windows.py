...
def test_cli_memory_overdue_retention_windows_classify_weekly_review(
    tmp_path: Path,
) -> None:
    ...
    assert result.command == "memory-overdue-retention-windows"
    assert result.payload["overdue_retention_window_counts"] == {"review_within_7d": 1}
    assert result.payload["highest_priority_overdue_retention_window"] == "review_within_7d"
    assert result.payload["highest_priority_overdue_retention_window_scope_kind"] == "repo"


def _seed_completed_session(database_path: Path, workspace_root: Path) -> SessionId:
    bootstrap = SessionBootstrapService().build(
        SessionBootstrapCommand(
            title="CLI memory overdue retention windows",
