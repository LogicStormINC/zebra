...
def test_cli_follow_through_verification_states_classify_admin_override(
    tmp_path: Path,
) -> None:
    ...
    assert result.command == "memory-overdue-retention-breach-follow-through-verification-states"
    assert result.payload["overdue_retention_breach_follow_through_verification_counts"] == {
...
def _seed_completed_session(database_path: Path, workspace_root: Path) -> SessionId:
    bootstrap = SessionBootstrapService().build(
        SessionBootstrapCommand(
            title="CLI memory overdue retention breach follow-through verification states",
