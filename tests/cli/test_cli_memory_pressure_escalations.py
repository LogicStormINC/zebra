...
def test_cli_memory_pressure_escalations_pick_stalled_scope_escalation(
    tmp_path: Path,
) -> None:
    ...
    stale_pending = _memory_record(
        memory_id="00000000-0000-0000-0000-000000000521",
    ...
    reviewed_user = _memory_record(
        memory_id="00000000-0000-0000-0000-000000000522",
        session_id=session_id,
    ...
    assert result.command == "memory-escalations"
    assert result.payload["escalation_recommendation_counts"] == {
        "escalate_stalled_scope": 1,
        "no_escalation_needed": 1,
    }
    assert (
        result.payload["highest_priority_escalation_recommendation"]
        == "escalate_stalled_scope"
    )
    assert result.payload["highest_priority_escalation_priority"] == "high"
    assert result.payload["highest_priority_escalation_scope_kind"] == "repo"


def _seed_completed_session(database_path: Path, workspace_root: Path) -> SessionId:
    bootstrap = SessionBootstrapService().build(
        SessionBootstrapCommand(
            title="CLI memory pressure escalations",
