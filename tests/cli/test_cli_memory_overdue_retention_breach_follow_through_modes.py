...
def test_cli_memory_overdue_retention_breach_follow_through_modes_classify_admin_override(
    tmp_path: Path,
) -> None:
    ...
    assert result.command == "memory-overdue-retention-breach-follow-through-modes"
    assert result.payload["overdue_retention_breach_follow_through_counts"] == {
        "admin_override_follow_through": 1
    }
    assert result.payload["highest_priority_overdue_retention_breach_follow_through_mode"] == (
        "admin_override_follow_through"
    )
    assert (
        result.payload["highest_priority_overdue_retention_breach_follow_through_scope_kind"]
        == "repo"
    )
...
def _seed_completed_session(database_path: Path, workspace_root: Path) -> SessionId:
    bootstrap = SessionBootstrapService().build(
        SessionBootstrapCommand(
            title="CLI memory overdue retention breach follow-through modes",
