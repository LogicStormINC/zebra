...
def test_cli_memory_overdue_recovery_paths_classify_next_local_review_recovery(
    tmp_path: Path,
) -> None:
    ...
    SQLiteMemoryStore(database_path).upsert(
        _memory_record(
            memory_id="00000000-0000-0000-0000-000000001001",
    ...
    assert result.command == "memory-overdue-recovery-paths"
    assert result.payload["overdue_recovery_path_counts"] == {
        "next_local_review_recovery": 1
    }
    assert result.payload["highest_priority_overdue_recovery_path"] == (
        "next_local_review_recovery"
    )
    assert result.payload["highest_priority_overdue_recovery_scope_kind"] == "repo"


def _seed_completed_session(database_path: Path, workspace_root: Path) -> SessionId:
    bootstrap = SessionBootstrapService().build(
        SessionBootstrapCommand(
            title="CLI memory overdue recovery paths",
