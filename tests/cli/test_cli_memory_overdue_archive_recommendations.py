...
def test_cli_memory_overdue_archive_recommendations_classify_revisit_after_next_review(
    tmp_path: Path,
) -> None:
    ...
    SQLiteMemoryStore(database_path).upsert(
        _memory_record(
            memory_id="00000000-0000-0000-0000-000000001121",
    ...
    assert result.command == "memory-overdue-archive-recommendations"
    assert result.payload["overdue_archive_recommendation_counts"] == {
        "revisit_archive_after_next_review": 1
    }
    assert result.payload["highest_priority_overdue_archive_recommendation"] == (
        "revisit_archive_after_next_review"
    )
    assert result.payload["highest_priority_overdue_archive_scope_kind"] == "repo"


def _seed_completed_session(database_path: Path, workspace_root: Path) -> SessionId:
    bootstrap = SessionBootstrapService().build(
        SessionBootstrapCommand(
            title="CLI memory overdue archive recommendations",
