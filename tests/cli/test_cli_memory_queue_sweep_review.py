...
def test_cli_memory_review_queue_filters_to_current_session(tmp_path: Path) -> None:
    ...
    workspace.mkdir()
    session_id = _seed_completed_session(database_path, workspace, title="cli-queue-a")
    other_session_id = _seed_completed_session(
        database_path, workspace, title="cli-queue-b"
    )
    ...
    assert result.command == "memory-review-queue"
    assert result.payload["session_id"] == str(session_id)
    assert result.payload["queue_sweep"] is True
...
def test_cli_memory_user_review_queue_expires_candidates(tmp_path: Path) -> None:
    database_path = tmp_path / "memory.sqlite"
    workspace = tmp_path / "workspace"
    workspace.mkdir()
    session_id = _seed_completed_session(database_path, workspace, title="cli-user-queue")
    candidate = _candidate_record(
        memory_id="00000000-0000-0000-0000-000000000243",
        session_id=session_id,
    ...
    assert result.command == "memory-user-review-queue"
