...
def test_cli_memory_overdue_resolution_checkpoints_classify_next_review_confirmation(
    tmp_path: Path,
) -> None:
    ...
    SQLiteMemoryStore(database_path).upsert(
        _memory_record(
            memory_id="00000000-0000-0000-0000-000000001031",
    ...
    assert result.command == "memory-overdue-resolution-checkpoints"
    assert result.payload["overdue_resolution_checkpoint_counts"] == {
        "next_review_confirmation_checkpoint": 1
    }
    assert result.payload["highest_priority_overdue_resolution_checkpoint"] == (
        "next_review_confirmation_checkpoint"
    )
    assert result.payload["highest_priority_overdue_resolution_scope_kind"] == "repo"


def _seed_completed_session(database_path: Path, workspace_root: Path) -> SessionId:
    bootstrap = SessionBootstrapService().build(
        SessionBootstrapCommand(
            title="CLI memory overdue resolution checkpoints",
