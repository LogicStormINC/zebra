...
def test_cli_memory_overdue_age_buckets_classify_repo_and_user_scopes(
    tmp_path: Path,
) -> None:
    ...
    stale_pending = _memory_record(
        memory_id="00000000-0000-0000-0000-000000000821",
    ...
    reviewed_user = _memory_record(
        memory_id="00000000-0000-0000-0000-000000000822",
        session_id=session_id,
    ...
    assert result.command == "memory-overdue-age-buckets"
    assert result.payload["overdue_age_bucket_counts"] == {
        "lt_1d_overdue": 1,
        "not_overdue": 1,
    }
    assert result.payload["highest_priority_overdue_age_bucket"] == "lt_1d_overdue"
    assert result.payload["highest_priority_overdue_age_scope_kind"] == "repo"


def _seed_completed_session(database_path: Path, workspace_root: Path) -> SessionId:
    bootstrap = SessionBootstrapService().build(
        SessionBootstrapCommand(
            title="CLI memory overdue age buckets",
