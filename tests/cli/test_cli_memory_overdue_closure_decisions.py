...
def test_cli_memory_overdue_closure_decisions_classify_hold_for_next_review_confirmation(
    tmp_path: Path,
) -> None:
    ...
    SQLiteMemoryStore(database_path).upsert(
        _memory_record(
            memory_id="00000000-0000-0000-0000-000000001091",
    ...
    assert result.command == "memory-overdue-closure-decisions"
    assert result.payload["overdue_closure_decision_counts"] == {
        "hold_for_next_review_confirmation": 1
    }
    assert result.payload["highest_priority_overdue_closure_decision"] == (
        "hold_for_next_review_confirmation"
    )
    assert result.payload["highest_priority_overdue_closure_scope_kind"] == "repo"


def _seed_completed_session(database_path: Path, workspace_root: Path) -> SessionId:
    bootstrap = SessionBootstrapService().build(
        SessionBootstrapCommand(
            title="CLI memory overdue closure decisions",
