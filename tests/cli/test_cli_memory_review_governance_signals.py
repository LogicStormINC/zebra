...
def test_cli_memory_review_governance_signals_report_backlog_and_reviews(
    tmp_path: Path,
) -> None:
    ...
    reviewed = _memory_record(
        memory_id="00000000-0000-0000-0000-000000000302",
        session_id=session_id,
    ...
    assert result.command == "memory-governance"
    assert result.payload["scope_count"] == 2
    assert result.payload["total_pending_count"] == 1
    assert result.payload["total_reviewed_count"] == 1
    assert result.payload["review_status_totals"] == {"confirmed": 1}


def _seed_completed_session(database_path: Path, workspace_root: Path) -> SessionId:
    bootstrap = SessionBootstrapService().build(
        SessionBootstrapCommand(
            title="CLI memory governance signals",
