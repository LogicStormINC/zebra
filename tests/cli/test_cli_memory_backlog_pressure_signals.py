...
def test_cli_memory_backlog_pressure_signals_classify_highest_pressure_scope(
    tmp_path: Path,
) -> None:
    ...
    store.upsert(
        _memory_record(
            memory_id="00000000-0000-0000-0000-000000000391",
    ...
    reviewed_user = _memory_record(
        memory_id="00000000-0000-0000-0000-000000000392",
        session_id=session_id,
    ...
    assert result.command == "memory-pressure"
    assert result.payload["pressure_level_counts"] == {"high": 1, "clear": 1}
    assert result.payload["highest_pressure_level"] == "high"
    assert result.payload["highest_pressure_scope_kind"] == "repo"
...
def _seed_completed_session(database_path: Path, workspace_root: Path) -> SessionId:
    bootstrap = SessionBootstrapService().build(
        SessionBootstrapCommand(
            title="CLI memory pressure signals",
