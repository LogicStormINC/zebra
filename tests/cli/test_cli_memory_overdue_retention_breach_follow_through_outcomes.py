...
def test_cli_memory_overdue_retention_breach_follow_through_outcomes_classify_admin_override(
    tmp_path: Path,
) -> None:
    ...
    assert result.command == "memory-overdue-retention-breach-follow-through-outcomes"
    assert result.payload["overdue_retention_breach_follow_through_outcome_counts"] == {
        "awaiting_admin_override_follow_through": 1
    }
    assert result.payload["highest_priority_overdue_retention_breach_follow_through_outcome"] == (
...
def _seed_completed_session(database_path: Path, workspace_root: Path) -> SessionId:
    bootstrap = SessionBootstrapService().build(
        SessionBootstrapCommand(
            title="CLI memory overdue retention breach follow-through outcomes",
