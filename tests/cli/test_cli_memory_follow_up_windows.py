...
def test_cli_memory_follow_up_windows_pick_immediate_stalled_scope_follow_up(
    tmp_path: Path,
) -> None:
    ...
    stale_pending = _memory_record(
        memory_id="00000000-0000-0000-0000-000000000621",
    ...
    reviewed_user = _memory_record(
        memory_id="00000000-0000-0000-0000-000000000622",
        session_id=session_id,
    ...
    assert result.command == "memory-follow-up-windows"
    assert result.payload["follow_up_window_counts"] == {
        "immediate_follow_up": 1,
        "next_7d_review_window": 1,
    }
    assert result.payload["highest_priority_follow_up_window"] == "immediate_follow_up"
    assert result.payload["highest_priority_follow_up_priority"] == "high"
    assert result.payload["highest_priority_follow_up_scope_kind"] == "repo"


def _seed_completed_session(database_path: Path, workspace_root: Path) -> SessionId:
    bootstrap = SessionBootstrapService().build(
        SessionBootstrapCommand(
            title="CLI memory follow up windows",
