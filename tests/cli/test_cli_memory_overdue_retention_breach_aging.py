...
def test_cli_memory_overdue_retention_breach_aging_classify_gte_7d(
    tmp_path: Path,
) -> None:
    ...
    assert result.command == "memory-overdue-retention-breach-aging"
    assert result.payload["overdue_retention_breach_age_bucket_counts"] == {
        "gte_7d_breached": 1
    }
    assert result.payload["highest_priority_overdue_retention_breach_age_bucket"] == (
        "gte_7d_breached"
    )
    assert result.payload["highest_priority_overdue_retention_breach_age_scope_kind"] == "repo"


def _seed_completed_session(database_path: Path, workspace_root: Path) -> SessionId:
    bootstrap = SessionBootstrapService().build(
        SessionBootstrapCommand(
            title="CLI memory overdue retention breach aging",
