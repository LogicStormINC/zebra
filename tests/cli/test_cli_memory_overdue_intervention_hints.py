...
def test_cli_memory_overdue_intervention_hints_classify_next_window_queue(
    tmp_path: Path,
) -> None:
    ...
    SQLiteMemoryStore(database_path).upsert(
        _memory_record(
            memory_id="00000000-0000-0000-0000-000000000941",
    ...
    assert result.command == "memory-overdue-interventions"
    assert result.payload["overdue_intervention_hint_counts"] == {
        "queue_next_review_window": 1
    }
    assert result.payload["highest_priority_overdue_intervention_hint"] == (
        "queue_next_review_window"
    )
    assert result.payload["highest_priority_overdue_intervention_scope_kind"] == "repo"


def _seed_completed_session(database_path: Path, workspace_root: Path) -> SessionId:
    bootstrap = SessionBootstrapService().build(
        SessionBootstrapCommand(
            title="CLI memory overdue intervention hints",
