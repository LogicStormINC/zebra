...
def test_cli_memory_overdue_type_rollups_aggregate_repo_types_for_overdue_scope(
    tmp_path: Path,
) -> None:
    ...
    store.upsert(
        _memory_record(
            memory_id="00000000-0000-0000-0000-000000000851",
    ...
    store.upsert(
        _memory_record(
            memory_id="00000000-0000-0000-0000-000000000852",
            session_id=session_id,
            visibility=MemoryVisibility.REPO,
            memory_type=MemoryType.PROCEDURE,
    ...
    store.upsert(
        _memory_record(
            memory_id="00000000-0000-0000-0000-000000000853",
            session_id=session_id,
    ...
    assert result.command == "memory-overdue-types"
    assert result.payload["overdue_scope_count"] == 1
    assert result.payload["overdue_memory_type_counts"] == {"procedure": 2}
    assert result.payload["highest_priority_overdue_memory_type"] == "procedure"
    assert result.payload["highest_priority_overdue_type_scope_kind"] == "repo"


def _seed_completed_session(database_path: Path, workspace_root: Path) -> SessionId:
    bootstrap = SessionBootstrapService().build(
        SessionBootstrapCommand(
            title="CLI memory overdue type rollups",
