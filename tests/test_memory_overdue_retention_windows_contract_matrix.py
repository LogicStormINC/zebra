...
def test_memory_overdue_retention_windows_contract_matrix_matches_api_and_cli(
    tmp_path: Path,
) -> None:
    ...
    api_response = RouteAdapter(create_app(database_path)).handle(
        RouteRequest(
            method="POST",
            path=f"/sessions/{api_session_id}/memory-overdue-retention-windows",
    ...
    assert cli_result.command == "memory-overdue-retention-windows"
...
def _contract_summary(payload: dict[str, object]) -> dict[str, object]:
    return {
        ...
        "overdue_retention_window_counts": payload["overdue_retention_window_counts"],
        "highest_priority_overdue_retention_window": payload[
            "highest_priority_overdue_retention_window"
        ],
        "highest_priority_overdue_retention_window_priority": payload[
            "highest_priority_overdue_retention_window_priority"
        ],
        "highest_priority_overdue_retention_window_scope_kind": payload[
            "highest_priority_overdue_retention_window_scope_kind"
        ],
        "highest_priority_overdue_retention_window_due_at": payload[
...
def _seed_completed_session(database_path: Path, workspace_root: Path) -> SessionId:
    bootstrap = SessionBootstrapService().build(
        SessionBootstrapCommand(
            title="Memory overdue retention windows matrix",
