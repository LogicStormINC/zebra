...
def test_retention_breach_follow_through_outcomes_contract_matrix_matches_api_and_cli(
    tmp_path: Path,
) -> None:
    ...
    api_response = RouteAdapter(create_app(database_path)).handle(
        RouteRequest(
            method="POST",
            path=f"/sessions/{api_session_id}/memory-overdue-retention-breach-follow-through-outcomes",
    ...
    assert cli_result.command == "memory-overdue-retention-breach-follow-through-outcomes"
...
def _contract_summary(payload: dict[str, object]) -> dict[str, object]:
    return {
        ...
        "overdue_retention_breach_follow_through_outcome_counts": payload[
            "overdue_retention_breach_follow_through_outcome_counts"
        ],
        "highest_priority_overdue_retention_breach_follow_through_outcome": payload[
            "highest_priority_overdue_retention_breach_follow_through_outcome"
        ],
        "highest_priority_overdue_retention_breach_follow_through_outcome_scope_kind": payload[
            "highest_priority_overdue_retention_breach_follow_through_outcome_scope_kind"
        ],
        "highest_priority_overdue_retention_breach_follow_through_outcome_memory_id": payload[
...
def _seed_completed_session(database_path: Path, workspace_root: Path) -> SessionId:
    bootstrap = SessionBootstrapService().build(
        SessionBootstrapCommand(
            title="Memory overdue retention breach follow-through outcomes matrix",
