...
def test_memory_pressure_escalations_contract_matrix_matches_across_api_and_cli(
    tmp_path: Path,
) -> None:
    ...
    api_pending = _memory_record(
        memory_id="00000000-0000-0000-0000-000000000531",
    ...
    cli_pending = _memory_record(
        memory_id="00000000-0000-0000-0000-000000000532",
    ...
    http_response = TestClient(create_http_app(api_database_path)).post(
        f"/sessions/{api_session_id}/memory-escalations",
...
def _normalize_http(payload: dict[str, object]) -> dict[str, object]:
    return {
        ...
        "escalation_recommendation_counts": payload["escalation_recommendation_counts"],
        "highest_priority_escalation_recommendation": payload[
            "highest_priority_escalation_recommendation"
        ],
        "highest_priority_escalation_priority": payload[
            "highest_priority_escalation_priority"
        ],
        "highest_priority_escalation_scope_kind": payload[
...
def _seed_completed_session(database_path: Path, workspace_root: Path) -> SessionId:
    bootstrap = SessionBootstrapService().build(
        SessionBootstrapCommand(
            title="Memory escalations matrix",
