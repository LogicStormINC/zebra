...
def test_memory_overdue_flags_contract_matrix_matches_across_api_and_cli(
    tmp_path: Path,
) -> None:
    ...
    api_pending = _memory_record(
        memory_id="00000000-0000-0000-0000-000000000731",
    ...
    cli_pending = _memory_record(
        memory_id="00000000-0000-0000-0000-000000000732",
    ...
    http_response = TestClient(create_http_app(api_database_path)).post(
        f"/sessions/{api_session_id}/memory-overdue-flags",
...
def _normalize_http(payload: dict[str, object]) -> dict[str, object]:
    return {
        ...
        "highest_priority_overdue_scope_kind": payload["highest_priority_overdue_scope_kind"],
        "highest_priority_overdue_priority": payload["highest_priority_overdue_priority"],
    }
...
def _seed_completed_session(database_path: Path, workspace_root: Path) -> SessionId:
    bootstrap = SessionBootstrapService().build(
        SessionBootstrapCommand(
            title="Memory overdue flags matrix",
