...
def test_memory_overdue_retention_breaches_contract_matrix_matches_api_and_cli(
    tmp_path: Path,
) -> None:
    ...
    api_response = RouteAdapter(create_app(database_path)).handle(
        RouteRequest(
            method="POST",
            path=f"/sessions/{api_session_id}/memory-overdue-retention-breaches",
            body={"as_of": "2026-08-07T06:00:00+00:00"},
        )
    )
    cli_result = execute(
        [
    ...
    assert cli_result.command == "memory-overdue-retention-breaches"
...
def _contract_summary(payload: dict[str, object]) -> dict[str, object]:
    return {
        ...
        "overdue_retention_breach_counts": payload["overdue_retention_breach_counts"],
        "highest_priority_overdue_retention_breach": payload[
            "highest_priority_overdue_retention_breach"
        ],
        "highest_priority_overdue_retention_breach_priority": payload[
            "highest_priority_overdue_retention_breach_priority"
        ],
        "highest_priority_overdue_retention_breach_scope_kind": payload[
            "highest_priority_overdue_retention_breach_scope_kind"
        ],
        "highest_priority_overdue_retention_breach_due_at": payload[
...
def _seed_completed_session(database_path: Path, workspace_root: Path) -> SessionId:
    bootstrap = SessionBootstrapService().build(
        SessionBootstrapCommand(
            title="Memory overdue retention breaches matrix",
