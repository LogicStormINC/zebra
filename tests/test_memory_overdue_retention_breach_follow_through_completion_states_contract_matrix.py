...
def test_follow_through_completion_states_contract_matrix_matches_api_and_cli(
    tmp_path: Path,
) -> None:
    ...
    assert cli_result.command == "memory-overdue-retention-breach-follow-through-completion-states"
...
def _contract_summary(payload: dict[str, object]) -> dict[str, object]:
    return {
        ...
        "overdue_retention_breach_follow_through_completion_counts": payload[
        ...
        "highest_priority_overdue_retention_breach_follow_through_completion_state": (
            payload["highest_priority_overdue_retention_breach_follow_through_completion_state"]
        ),
        "highest_priority_overdue_retention_breach_follow_through_completion_scope_kind": (
            payload["highest_priority_overdue_retention_breach_follow_through_completion_scope_kind"]
        ),
        "highest_priority_overdue_retention_breach_follow_through_completion_memory_id": (
            payload["highest_priority_overdue_retention_breach_follow_through_completion_memory_id"]
        ),
...
def _seed_completed_session(database_path: Path, workspace_root: Path) -> SessionId:
    bootstrap = SessionBootstrapService().build(
        SessionBootstrapCommand(
            title="Memory overdue retention breach follow-through completion states matrix",
            user_input="Inspect overdue retention breach follow-through completion states.",
