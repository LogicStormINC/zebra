...
def test_memory_overdue_resolution_outcomes_contract_matrix_matches_api_and_cli(
    tmp_path: Path,
) -> None:
    ...
    SQLiteMemoryStore(api_database_path).upsert(
        _memory_record(
            memory_id="00000000-0000-0000-0000-000000001071",
    ...
    SQLiteMemoryStore(cli_database_path).upsert(
        _memory_record(
            memory_id="00000000-0000-0000-0000-000000001072",
    ...
    http_response = TestClient(create_http_app(api_database_path)).post(
        f"/sessions/{api_session_id}/memory-overdue-resolution-outcomes",
...
def _normalize_http(payload: dict[str, object]) -> dict[str, object]:
    return {
        ...
        "overdue_resolution_outcome_counts": payload["overdue_resolution_outcome_counts"],
        "highest_priority_overdue_resolution_outcome": payload[
            "highest_priority_overdue_resolution_outcome"
        ],
        "highest_priority_overdue_resolution_outcome_priority": payload[
            "highest_priority_overdue_resolution_outcome_priority"
        ],
        "highest_priority_overdue_resolution_outcome_scope_kind": payload[
...
def _seed_completed_session(database_path: Path, workspace_root: Path) -> SessionId:
    bootstrap = SessionBootstrapService().build(
        SessionBootstrapCommand(
            title="Memory overdue resolution outcomes matrix",
