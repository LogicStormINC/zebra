...
def test_memory_overdue_escalation_lanes_contract_matrix_matches_api_and_cli(
    tmp_path: Path,
) -> None:
    ...
    SQLiteMemoryStore(api_database_path).upsert(
        _memory_record(
            memory_id="00000000-0000-0000-0000-000000000981",
    ...
    SQLiteMemoryStore(cli_database_path).upsert(
        _memory_record(
            memory_id="00000000-0000-0000-0000-000000000982",
    ...
    http_response = TestClient(create_http_app(api_database_path)).post(
        f"/sessions/{api_session_id}/memory-overdue-escalation-lanes",
...
def _normalize_http(payload: dict[str, object]) -> dict[str, object]:
    return {
        ...
        "overdue_escalation_lane_counts": payload["overdue_escalation_lane_counts"],
        "highest_priority_overdue_escalation_lane": payload[
            "highest_priority_overdue_escalation_lane"
        ],
        "highest_priority_overdue_escalation_priority": payload[
            "highest_priority_overdue_escalation_priority"
        ],
        "highest_priority_overdue_escalation_scope_kind": payload[
...
def _seed_completed_session(database_path: Path, workspace_root: Path) -> SessionId:
    bootstrap = SessionBootstrapService().build(
        SessionBootstrapCommand(
            title="Memory overdue escalation lanes matrix",
