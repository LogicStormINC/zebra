...
def test_memory_overdue_archive_recommendations_contract_matrix_matches_api_and_cli(
    tmp_path: Path,
) -> None:
    ...
    SQLiteMemoryStore(api_database_path).upsert(
        _memory_record(
            memory_id="00000000-0000-0000-0000-000000001131",
    ...
    SQLiteMemoryStore(cli_database_path).upsert(
        _memory_record(
            memory_id="00000000-0000-0000-0000-000000001132",
    ...
    http_response = TestClient(create_http_app(api_database_path)).post(
        f"/sessions/{api_session_id}/memory-overdue-archive-recommendations",
        json={"as_of": "2026-07-11T06:00:00+00:00"},
    )
    cli_result = execute(
        [
...
def _normalize_http(payload: dict[str, object]) -> dict[str, object]:
    return {
        ...
        "overdue_archive_recommendation_counts": payload[
            "overdue_archive_recommendation_counts"
        ],
        "highest_priority_overdue_archive_recommendation": payload[
            "highest_priority_overdue_archive_recommendation"
        ],
        "highest_priority_overdue_archive_priority": payload[
            "highest_priority_overdue_archive_priority"
        ],
        "highest_priority_overdue_archive_scope_kind": payload[
...
def _seed_completed_session(database_path: Path, workspace_root: Path) -> SessionId:
    bootstrap = SessionBootstrapService().build(
        SessionBootstrapCommand(
            title="Memory overdue archive recommendations matrix",
