...
def test_receipt_acceptance_survives_a_concurrently_admitted_message() -> None:
    """Review R8-P1: the durable projection may already include the next
    human message when a lost-commit response is reconciled — the receipt
    must be accepted at ITS revision instead of mismatching the ahead
    projection."""
    bootstrap = SessionBootstrapService().build(
        SessionBootstrapCommand(
            title="Receipt ahead projection",
            user_input="preference: reconcile after a concurrent message",
            workspace_root="/tmp/cloud-memory-ahead-projection",
        )
    )
    started = SessionEvent.create(
        session_id=bootstrap.session.session_id,
    ...
    # The next human message lands (durable stream AND projection advance
    # past the receipt revision) before the lost response is reconciled.
    follow_up = SessionEvent.create(
        session_id=bootstrap.session.session_id,
        sequence=completed.sequence + 1,
        event_type=EventType.USER_MESSAGE_RECEIVED,
        actor=EventActor.USER,
        payload={"content": "next message wins the race"},
    )
    ahead_session = apply_session_event(session, follow_up)
    event_store.events.append(follow_up)
    event_store.session = ahead_session
    ...
    result = finalize_cloud_memory(
        ...
        projection_store=_ProjectionStore(event_store),  # type: ignore[arg-type]
        workspace_store=_WorkspaceStore(event_store),  # type: ignore[arg-type]
        started_at=follow_up.created_at,
        allow_commit=False,
    )
