...
def test_memory_overdue_retention_guidance_contract_matrix_matches_api_and_cli(
    tmp_path: Path,
) -> None:
    ...
    api_response = RouteAdapter(create_app(database_path)).handle(
        RouteRequest(
            method="POST",
            path=f"/sessions/{api_session_id}/memory-overdue-retention-guidance",
            body={"as_of": "2026-07-11T06:00:00+00:00"},
        )
    )
    cli_result = execute(
        [
    ...
    assert cli_result.command == "memory-overdue-retention-guidance"

    api_summary = _contract_summary(api_response.body)
    cli_summary = _contract_summary(cli_result.payload)
...
def _contract_summary(payload: dict[str, object]) -> dict[str, object]:
    return {
        ...
        "overdue_retention_guidance_counts": payload["overdue_retention_guidance_counts"],
        "highest_priority_overdue_retention_guidance": payload[
            "highest_priority_overdue_retention_guidance"
        ],
        "highest_priority_overdue_retention_priority": payload[
            "highest_priority_overdue_retention_priority"
        ],
        "highest_priority_overdue_retention_scope_kind": payload[
            "highest_priority_overdue_retention_scope_kind"
        ],
        "highest_priority_overdue_retention_bucket": payload[
...
def _seed_completed_session(database_path: Path, workspace_root: Path) -> SessionId:
    bootstrap = SessionBootstrapService().build(
        SessionBootstrapCommand(
            title="Memory overdue retention guidance matrix",
