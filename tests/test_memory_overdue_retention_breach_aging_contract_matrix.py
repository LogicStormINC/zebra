...
def test_memory_overdue_retention_breach_aging_contract_matrix_matches_api_and_cli(
    tmp_path: Path,
) -> None:
    ...
    api_response = RouteAdapter(create_app(database_path)).handle(
        RouteRequest(
            method="POST",
            path=f"/sessions/{api_session_id}/memory-overdue-retention-breach-aging",
    ...
    assert cli_result.command == "memory-overdue-retention-breach-aging"
...
def _contract_summary(payload: dict[str, object]) -> dict[str, object]:
    return {
        ...
        "overdue_retention_breach_age_bucket_counts": payload[
            "overdue_retention_breach_age_bucket_counts"
        ],
        "highest_priority_overdue_retention_breach_age_bucket": payload[
            "highest_priority_overdue_retention_breach_age_bucket"
        ],
        "highest_priority_overdue_retention_breach_age_scope_kind": payload[
            "highest_priority_overdue_retention_breach_age_scope_kind"
        ],
        "highest_priority_overdue_retention_breach_age_days": payload[
...
def _seed_completed_session(database_path: Path, workspace_root: Path) -> SessionId:
    bootstrap = SessionBootstrapService().build(
        SessionBootstrapCommand(
            title="Memory overdue retention breach aging matrix",
