...
def test_memory_overdue_retention_breach_lanes_contract_matrix_matches_api_and_cli(
    tmp_path: Path,
) -> None:
    ...
    api_response = RouteAdapter(create_app(database_path)).handle(
        RouteRequest(
            method="POST",
            path=f"/sessions/{api_session_id}/memory-overdue-retention-breach-lanes",
    ...
    assert cli_result.command == "memory-overdue-retention-breach-lanes"
...
def _contract_summary(payload: dict[str, object]) -> dict[str, object]:
    return {
        ...
        "overdue_retention_breach_lane_counts": payload[
            "overdue_retention_breach_lane_counts"
        ],
        "highest_priority_overdue_retention_breach_lane": payload[
            "highest_priority_overdue_retention_breach_lane"
        ],
        "highest_priority_overdue_retention_breach_lane_scope_kind": payload[
            "highest_priority_overdue_retention_breach_lane_scope_kind"
        ],
        "highest_priority_overdue_retention_breach_lane_target_memory_id": payload[
...
def _seed_completed_session(database_path: Path, workspace_root: Path) -> SessionId:
    bootstrap = SessionBootstrapService().build(
        SessionBootstrapCommand(
            title="Memory overdue retention breach lanes matrix",
