...
def test_memory_overdue_age_buckets_contract_matrix_matches_across_api_and_cli(
    tmp_path: Path,
) -> None:
    ...
    api_pending = _memory_record(
        memory_id="00000000-0000-0000-0000-000000000831",
    ...
    cli_pending = _memory_record(
        memory_id="00000000-0000-0000-0000-000000000832",
    ...
    http_response = TestClient(create_http_app(api_database_path)).post(
        f"/sessions/{api_session_id}/memory-overdue-age-buckets",
...
def _normalize_http(payload: dict[str, object]) -> dict[str, object]:
    return {
        ...
        "overdue_age_bucket_counts": payload["overdue_age_bucket_counts"],
        "highest_priority_overdue_age_bucket": payload["highest_priority_overdue_age_bucket"],
        "highest_priority_overdue_age_scope_kind": payload[
            "highest_priority_overdue_age_scope_kind"
        ],
        "highest_priority_overdue_age_days": payload["highest_priority_overdue_age_days"],
    }
...
def _seed_completed_session(database_path: Path, workspace_root: Path) -> SessionId:
    bootstrap = SessionBootstrapService().build(
        SessionBootstrapCommand(
            title="Memory overdue age buckets matrix",
