...
def test_memory_overdue_resolution_checkpoints_contract_matrix_matches_api_and_cli(
    tmp_path: Path,
) -> None:
    ...
    SQLiteMemoryStore(api_database_path).upsert(
        _memory_record(
            memory_id="00000000-0000-0000-0000-000000001041",
    ...
    SQLiteMemoryStore(cli_database_path).upsert(
        _memory_record(
            memory_id="00000000-0000-0000-0000-000000001042",
    ...
    http_response = TestClient(create_http_app(api_database_path)).post(
        f"/sessions/{api_session_id}/memory-overdue-resolution-checkpoints",
...
def _normalize_http(payload: dict[str, object]) -> dict[str, object]:
    return {
        ...
        "overdue_resolution_checkpoint_counts": payload[
            "overdue_resolution_checkpoint_counts"
        ],
        "highest_priority_overdue_resolution_checkpoint": payload[
            "highest_priority_overdue_resolution_checkpoint"
        ],
        "highest_priority_overdue_resolution_priority": payload[
            "highest_priority_overdue_resolution_priority"
        ],
        "highest_priority_overdue_resolution_scope_kind": payload[
...
def _seed_completed_session(database_path: Path, workspace_root: Path) -> SessionId:
    bootstrap = SessionBootstrapService().build(
        SessionBootstrapCommand(
            title="Memory overdue resolution checkpoints matrix",
