...
def test_user_memory_queue_contract_matrix_matches_across_api_and_cli(
    tmp_path: Path,
) -> None:
    ...
    api_store.upsert(_memory_record("00000000-0000-0000-0000-000000000191", api_session_id))
    cli_store.upsert(_memory_record("00000000-0000-0000-0000-000000000192", cli_session_id))
    api_store.upsert(
        _memory_record(
            "00000000-0000-0000-0000-000000000193",
            api_session_id,
            status=MemoryStatus.CONFIRMED,
            text="Already confirmed preference.",
        )
    )
    cli_store.upsert(
        _memory_record(
    ...
    http_response = TestClient(create_http_app(api_database_path)).get("/users/user-1/memory/queue")
    cli_result = execute(["memory-user-queue", "user-1", "--database", str(cli_database_path)])
...
def _seed_session(database_path: Path, workspace_root: Path) -> SessionId:
    bootstrap = SessionBootstrapService().build(
        SessionBootstrapCommand(
            title="Memory scope queue matrix",
...
def _memory_record(
    ...
    *,
    ...
    text: str = "Prefer concise CLI output.",
) -> MemoryRecord:
