...
def test_memory_overdue_type_rollups_contract_matrix_matches_across_api_and_cli(
    tmp_path: Path,
) -> None:
    ...
    SQLiteMemoryStore(api_database_path).upsert(
        _memory_record(
            memory_id="00000000-0000-0000-0000-000000000861",
    ...
    SQLiteMemoryStore(api_database_path).upsert(
        _memory_record(
            memory_id="00000000-0000-0000-0000-000000000862",
            session_id=api_session_id,
            visibility=MemoryVisibility.REPO,
            memory_type=MemoryType.PROCEDURE,
    ...
    SQLiteMemoryStore(cli_database_path).upsert(
        _memory_record(
            memory_id="00000000-0000-0000-0000-000000000863",
    ...
    SQLiteMemoryStore(cli_database_path).upsert(
        _memory_record(
            memory_id="00000000-0000-0000-0000-000000000864",
            session_id=cli_session_id,
            visibility=MemoryVisibility.REPO,
            memory_type=MemoryType.PROCEDURE,
            text="Second pending repo memory.",
    ...
    http_response = TestClient(create_http_app(api_database_path)).post(
        f"/sessions/{api_session_id}/memory-overdue-types",
...
def _normalize_http(payload: dict[str, object]) -> dict[str, object]:
    return {
        ...
        "overdue_memory_type_counts": payload["overdue_memory_type_counts"],
        "highest_priority_overdue_memory_type": payload[
            "highest_priority_overdue_memory_type"
        ],
        "highest_priority_overdue_memory_type_count": payload[
            "highest_priority_overdue_memory_type_count"
        ],
        "highest_priority_overdue_type_scope_kind": payload[
...
def _seed_completed_session(database_path: Path, workspace_root: Path) -> SessionId:
    bootstrap = SessionBootstrapService().build(
        SessionBootstrapCommand(
            title="Memory overdue type rollups matrix",
