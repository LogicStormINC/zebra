...
def test_api_memory_overdue_retention_breach_aging_classify_gte_7d(
    tmp_path: Path,
) -> None:
    ...
    response = create_app(database_path).get_memory_overdue_retention_breach_aging(
        str(session_id),
        {"as_of": "2026-08-20T06:00:00+00:00"},
    )

    assert response.status_code == 200
    assert response.body["overdue_retention_breach_age_bucket_counts"] == {
        "gte_7d_breached": 1
    }
    assert response.body["highest_priority_overdue_retention_breach_age_bucket"] == (
        "gte_7d_breached"
    )
    assert response.body["highest_priority_overdue_retention_breach_age_scope_kind"] == "repo"
    assert response.body["scopes"][0]["overdue_retention_breach_age_bucket"] == (
        "gte_7d_breached"
    )


def test_route_adapter_handles_memory_overdue_retention_breach_aging(
    tmp_path: Path,
) -> None:
    database_path = tmp_path / "memory.sqlite"
    workspace = tmp_path / "workspace"
    ...
    response = RouteAdapter(create_app(database_path)).handle(
        RouteRequest(
            method="POST",
            path=f"/sessions/{session_id}/memory-overdue-retention-breach-aging",
...
def _seed_completed_session(database_path: Path, workspace_root: Path) -> SessionId:
    bootstrap = SessionBootstrapService().build(
        SessionBootstrapCommand(
            title="Memory overdue retention breach aging",
            user_input="Inspect overdue retention breach aging.",
