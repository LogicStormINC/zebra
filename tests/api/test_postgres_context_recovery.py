...
def test_postgres_recovery_uses_administrative_cas_and_preserves_http_contract(
    ...
    missing_namespace = RouteAdapter(create_app(tmp_path / "legacy.sqlite", stores=stores)).handle(
        RouteRequest(
            method="POST",
    ...
    ).handle(RouteRequest(method="POST", path=f"/sessions/{session.session_id}/context/compact"))
