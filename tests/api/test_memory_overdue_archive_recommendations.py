...
def test_api_memory_overdue_archive_recommendations_classify_revisit_after_next_review(
    tmp_path: Path,
) -> None:
    ...
    response = create_app(database_path).get_memory_overdue_archive_recommendations(
        str(session_id),
        {"as_of": "2026-07-11T06:00:00+00:00"},
    )

    assert response.status_code == 200
    assert response.body["overdue_archive_recommendation_counts"] == {
        "revisit_archive_after_next_review": 1
    }
    assert response.body["highest_priority_overdue_archive_recommendation"] == (
        "revisit_archive_after_next_review"
    )
    assert response.body["highest_priority_overdue_archive_priority"] == "low"
    assert response.body["highest_priority_overdue_archive_scope_kind"] == "repo"
    assert response.body["scopes"][0]["overdue_archive_recommendation"] == (
        "revisit_archive_after_next_review"
    )


def test_route_adapter_handles_memory_overdue_archive_recommendations(
    tmp_path: Path,
) -> None:
    database_path = tmp_path / "memory.sqlite"
    workspace = tmp_path / "workspace"
    ...
    response = RouteAdapter(create_app(database_path)).handle(
        RouteRequest(
            method="POST",
            path=f"/sessions/{session_id}/memory-overdue-archive-recommendations",
...
def _seed_completed_session(database_path: Path, workspace_root: Path) -> SessionId:
    bootstrap = SessionBootstrapService().build(
        SessionBootstrapCommand(
            title="Memory overdue archive recommendations",
            user_input="Inspect overdue archive recommendations.",
