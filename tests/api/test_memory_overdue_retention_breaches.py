...
def test_api_memory_overdue_retention_breaches_classify_weekly_breach(
    tmp_path: Path,
) -> None:
    ...
    response = create_app(database_path).get_memory_overdue_retention_breaches(
        str(session_id),
        {"as_of": "2026-08-07T06:00:00+00:00"},
    )

    assert response.status_code == 200
    assert response.body["overdue_retention_breach_counts"] == {
        "extended_window_breached": 1
    }
    assert response.body["highest_priority_overdue_retention_breach"] == (
        "extended_window_breached"
    )
    assert response.body["highest_priority_overdue_retention_breach_priority"] == "medium"
    assert response.body["highest_priority_overdue_retention_breach_scope_kind"] == "repo"
    assert response.body["scopes"][0]["overdue_retention_breach"] == "extended_window_breached"


def test_route_adapter_handles_memory_overdue_retention_breaches(
    tmp_path: Path,
) -> None:
    database_path = tmp_path / "memory.sqlite"
    workspace = tmp_path / "workspace"
    ...
    response = RouteAdapter(create_app(database_path)).handle(
        RouteRequest(
            method="POST",
            path=f"/sessions/{session_id}/memory-overdue-retention-breaches",
            body={"tenant_id": "tenant-1", "as_of": "2026-08-07T06:00:00+00:00"},
        )
    )
...
def _seed_completed_session(database_path: Path, workspace_root: Path) -> SessionId:
    bootstrap = SessionBootstrapService().build(
        SessionBootstrapCommand(
            title="Memory overdue retention breaches",
            user_input="Inspect overdue retention breaches.",
