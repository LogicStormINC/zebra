...
def test_api_memory_overdue_retention_guidance_classify_next_review_retention(
    tmp_path: Path,
) -> None:
    ...
    response = create_app(database_path).get_memory_overdue_retention_guidance(
        str(session_id),
        {"as_of": "2026-07-11T06:00:00+00:00"},
    )

    assert response.status_code == 200
    assert response.body["overdue_retention_guidance_counts"] == {
        "retain_until_next_review": 1
    }
    assert response.body["highest_priority_overdue_retention_guidance"] == (
        "retain_until_next_review"
    )
    assert response.body["highest_priority_overdue_retention_priority"] == "low"
    assert response.body["highest_priority_overdue_retention_bucket"] == "standard"
    assert response.body["highest_priority_overdue_retention_scope_kind"] == "repo"
    assert response.body["scopes"][0]["overdue_retention_guidance"] == (
        "retain_until_next_review"
    )


def test_route_adapter_handles_memory_overdue_retention_guidance(
    tmp_path: Path,
) -> None:
    database_path = tmp_path / "memory.sqlite"
    workspace = tmp_path / "workspace"
    ...
    response = RouteAdapter(create_app(database_path)).handle(
        RouteRequest(
            method="POST",
            path=f"/sessions/{session_id}/memory-overdue-retention-guidance",
...
def _seed_completed_session(database_path: Path, workspace_root: Path) -> SessionId:
    bootstrap = SessionBootstrapService().build(
        SessionBootstrapCommand(
            title="Memory overdue retention guidance",
            user_input="Inspect overdue retention guidance.",
