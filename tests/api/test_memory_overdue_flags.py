...
def test_api_memory_overdue_flags_pick_high_priority_overdue_scope(tmp_path: Path) -> None:
    ...
    reviewed_user = _memory_record(
        memory_id="00000000-0000-0000-0000-000000000712",
        session_id=session_id,
    ...
    response = create_app(database_path).get_memory_follow_up_overdue_flags(
        str(session_id),
    ...
    assert response.body["highest_priority_overdue_scope_kind"] == "repo"
    assert response.body["highest_priority_overdue_priority"] == "high"
    assert response.body["highest_priority_overdue_since"] == "2026-07-09T00:00:00+00:00"
    assert response.body["highest_priority_overdue_target_memory_id"] == str(
        stale_pending.memory_id
    )
    repo_scope = response.body["scopes"][0]
    user_scope = response.body["scopes"][1]
    assert repo_scope["follow_up_overdue"] is True
    assert repo_scope["follow_up_overdue_priority"] == "high"
    assert user_scope["follow_up_overdue"] is False


def test_route_adapter_handles_memory_overdue_flags(tmp_path: Path) -> None:
    database_path = tmp_path / "memory.sqlite"
    workspace = tmp_path / "workspace"
    workspace.mkdir()
    session_id = _seed_completed_session(database_path, workspace)
    ...
    response = RouteAdapter(create_app(database_path)).handle(
        RouteRequest(
            method="POST",
            path=f"/sessions/{session_id}/memory-overdue-flags",
