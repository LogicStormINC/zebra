...
def test_api_memory_overdue_closure_decisions_classify_hold_for_next_review_confirmation(
    tmp_path: Path,
) -> None:
    ...
    SQLiteMemoryStore(database_path).upsert(
        _memory_record(
            memory_id="00000000-0000-0000-0000-000000001081",
    ...
    response = create_app(database_path).get_memory_overdue_closure_decisions(
        str(session_id),
        {"as_of": "2026-07-11T06:00:00+00:00"},
    )

    assert response.status_code == 200
    assert response.body["overdue_closure_decision_counts"] == {
        "hold_for_next_review_confirmation": 1
    }
    assert response.body["highest_priority_overdue_closure_decision"] == (
        "hold_for_next_review_confirmation"
    )
    assert response.body["highest_priority_overdue_closure_priority"] == "low"
    assert response.body["highest_priority_overdue_closure_scope_kind"] == "repo"
    assert response.body["scopes"][0]["overdue_closure_decision"] == (
        "hold_for_next_review_confirmation"
    )


def test_route_adapter_handles_memory_overdue_closure_decisions(tmp_path: Path) -> None:
    database_path = tmp_path / "memory.sqlite"
    workspace = tmp_path / "workspace"
    workspace.mkdir()
    session_id = _seed_completed_session(database_path, workspace)
    ...
    response = RouteAdapter(create_app(database_path)).handle(
        RouteRequest(
            method="POST",
            path=f"/sessions/{session_id}/memory-overdue-closure-decisions",
...
def _seed_completed_session(database_path: Path, workspace_root: Path) -> SessionId:
    bootstrap = SessionBootstrapService().build(
        SessionBootstrapCommand(
            title="Memory overdue closure decisions",
            user_input="Inspect overdue closure decisions.",
