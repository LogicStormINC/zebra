...
def test_api_memory_overdue_recovery_paths_classify_next_local_review_recovery(
    tmp_path: Path,
) -> None:
    ...
    SQLiteMemoryStore(database_path).upsert(
        _memory_record(
            memory_id="00000000-0000-0000-0000-000000000991",
    ...
    response = create_app(database_path).get_memory_overdue_recovery_paths(
        str(session_id),
        {"as_of": "2026-07-11T06:00:00+00:00"},
    )

    assert response.status_code == 200
    assert response.body["overdue_recovery_path_counts"] == {
        "next_local_review_recovery": 1
    }
    assert response.body["highest_priority_overdue_recovery_path"] == (
        "next_local_review_recovery"
    )
    assert response.body["highest_priority_overdue_recovery_priority"] == "low"
    assert response.body["highest_priority_overdue_recovery_scope_kind"] == "repo"
    assert response.body["scopes"][0]["overdue_recovery_path"] == (
        "next_local_review_recovery"
    )


def test_route_adapter_handles_memory_overdue_recovery_paths(tmp_path: Path) -> None:
    database_path = tmp_path / "memory.sqlite"
    workspace = tmp_path / "workspace"
    workspace.mkdir()
    session_id = _seed_completed_session(database_path, workspace)
    ...
    response = RouteAdapter(create_app(database_path)).handle(
        RouteRequest(
            method="POST",
            path=f"/sessions/{session_id}/memory-overdue-recovery-paths",
...
def _seed_completed_session(database_path: Path, workspace_root: Path) -> SessionId:
    bootstrap = SessionBootstrapService().build(
        SessionBootstrapCommand(
            title="Memory overdue recovery paths",
            user_input="Inspect overdue recovery paths.",
