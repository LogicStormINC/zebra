...
def test_api_memory_overdue_visibility_rollups_aggregate_scope_visibility_counts(
    tmp_path: Path,
) -> None:
    ...
    store.upsert(
        _memory_record(
            memory_id="00000000-0000-0000-0000-000000000871",
    ...
    store.upsert(
        _memory_record(
            memory_id="00000000-0000-0000-0000-000000000872",
            session_id=session_id,
            visibility=MemoryVisibility.REPO,
            memory_type=MemoryType.PROCEDURE,
    ...
    store.upsert(
        _memory_record(
            memory_id="00000000-0000-0000-0000-000000000873",
            session_id=session_id,
    ...
    response = create_app(database_path).get_memory_overdue_visibility_rollups(
        str(session_id),
    ...
    assert response.body["overdue_memory_visibility_counts"] == {"repo": 2}
    assert response.body["highest_priority_overdue_memory_visibility"] == "repo"
    assert response.body["highest_priority_overdue_memory_visibility_count"] == 2
    assert response.body["highest_priority_overdue_visibility_scope_kind"] == "repo"
    assert response.body["highest_priority_overdue_target_memory_visibility"] == "repo"
    repo_scope = response.body["scopes"][0]
    user_scope = response.body["scopes"][1]
    assert repo_scope["overdue_memory_visibility_counts"] == {"repo": 2}
    assert user_scope["overdue_memory_visibility_counts"] == {}


def test_route_adapter_handles_memory_overdue_visibility(tmp_path: Path) -> None:
    database_path = tmp_path / "memory.sqlite"
    workspace = tmp_path / "workspace"
    workspace.mkdir()
    session_id = _seed_completed_session(database_path, workspace)
    ...
    response = RouteAdapter(create_app(database_path)).handle(
        RouteRequest(
            method="POST",
            path=f"/sessions/{session_id}/memory-overdue-visibility",
...
def _seed_completed_session(database_path: Path, workspace_root: Path) -> SessionId:
    bootstrap = SessionBootstrapService().build(
        SessionBootstrapCommand(
            title="Memory overdue visibility rollups",
