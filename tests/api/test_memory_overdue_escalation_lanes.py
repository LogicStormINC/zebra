...
def test_api_memory_overdue_escalation_lanes_classify_local_queue_lane(
    tmp_path: Path,
) -> None:
    ...
    SQLiteMemoryStore(database_path).upsert(
        _memory_record(
            memory_id="00000000-0000-0000-0000-000000000961",
    ...
    response = create_app(database_path).get_memory_overdue_escalation_lanes(
        str(session_id),
        {"as_of": "2026-07-11T06:00:00+00:00"},
    )

    assert response.status_code == 200
    assert response.body["overdue_escalation_lane_counts"] == {"local_queue_lane": 1}
    assert response.body["highest_priority_overdue_escalation_lane"] == "local_queue_lane"
    assert response.body["highest_priority_overdue_escalation_priority"] == "low"
    assert response.body["highest_priority_overdue_escalation_scope_kind"] == "repo"
    assert response.body["scopes"][0]["overdue_escalation_lane"] == "local_queue_lane"


def test_route_adapter_handles_memory_overdue_escalation_lanes(tmp_path: Path) -> None:
    database_path = tmp_path / "memory.sqlite"
    workspace = tmp_path / "workspace"
    workspace.mkdir()
    session_id = _seed_completed_session(database_path, workspace)
    ...
    response = RouteAdapter(create_app(database_path)).handle(
        RouteRequest(
            method="POST",
            path=f"/sessions/{session_id}/memory-overdue-escalation-lanes",
...
def _seed_completed_session(database_path: Path, workspace_root: Path) -> SessionId:
    bootstrap = SessionBootstrapService().build(
        SessionBootstrapCommand(
            title="Memory overdue escalation lanes",
            user_input="Inspect overdue escalation lanes.",
