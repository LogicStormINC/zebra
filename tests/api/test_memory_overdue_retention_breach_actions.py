...
def test_api_memory_overdue_retention_breach_actions_classify_force_archive(
    tmp_path: Path,
) -> None:
    ...
    response = create_app(database_path).get_memory_overdue_retention_breach_actions(
        str(session_id),
        {"as_of": "2026-08-20T06:00:00+00:00"},
    )

    assert response.status_code == 200
    assert response.body["overdue_retention_breach_action_counts"] == {
        "force_archive_or_override": 1
    }
    assert response.body["highest_priority_overdue_retention_breach_action"] == (
        "force_archive_or_override"
    )
    assert response.body["highest_priority_overdue_retention_breach_action_priority"] == "high"
    assert response.body["highest_priority_overdue_retention_breach_action_scope_kind"] == "repo"
    assert response.body["scopes"][0]["overdue_retention_breach_action"] == (
        "force_archive_or_override"
    )


def test_route_adapter_handles_memory_overdue_retention_breach_actions(
    tmp_path: Path,
) -> None:
    database_path = tmp_path / "memory.sqlite"
    workspace = tmp_path / "workspace"
    ...
    response = RouteAdapter(create_app(database_path)).handle(
        RouteRequest(
            method="POST",
            path=f"/sessions/{session_id}/memory-overdue-retention-breach-actions",
...
def _seed_completed_session(database_path: Path, workspace_root: Path) -> SessionId:
    bootstrap = SessionBootstrapService().build(
        SessionBootstrapCommand(
            title="Memory overdue retention breach actions",
            user_input="Inspect overdue retention breach actions.",
