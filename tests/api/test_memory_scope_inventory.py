...
def test_api_get_user_memory_returns_scoped_inventory(tmp_path: Path) -> None:
    ...
    response = create_app(database_path).get_user_memory("user-1")
...
def test_route_adapter_handles_tenant_memory_inventory(tmp_path: Path) -> None:
    database_path = tmp_path / "memory.sqlite"
    workspace = tmp_path / "workspace"
    workspace.mkdir()
    session_id = _seed_session(database_path, workspace)
    ...
    response = RouteAdapter(create_app(database_path)).handle(
        RouteRequest(method="GET", path="/tenants/tenant-1/memory")
    )
...
def _memory_record(
    *,
    ...
    return MemoryRecord(
        memory_id=MemoryId(UUID("00000000-0000-0000-0000-000000000161")),
