...
def test_api_memory_pressure_escalations_pick_stalled_scope_escalation(
    tmp_path: Path,
) -> None:
    ...
    stale_pending = _memory_record(
        memory_id="00000000-0000-0000-0000-000000000511",
    ...
    reviewed_user = _memory_record(
        memory_id="00000000-0000-0000-0000-000000000512",
        session_id=session_id,
    ...
    response = create_app(database_path).get_memory_pressure_escalation_recommendations(
        str(session_id),
        {"user_id": "user-1", "as_of": "2026-07-09T00:00:00+00:00"},
    )

    assert response.status_code == 200
    assert response.body["escalation_recommendation_counts"] == {
        "escalate_stalled_scope": 1,
        "no_escalation_needed": 1,
    }
    assert (
        response.body["highest_priority_escalation_recommendation"]
        == "escalate_stalled_scope"
    )
    assert response.body["highest_priority_escalation_priority"] == "high"
    assert response.body["highest_priority_escalation_scope_kind"] == "repo"
    assert response.body["highest_priority_escalation_target_memory_id"] == str(
        stale_pending.memory_id
    )
    repo_scope = response.body["scopes"][0]
    user_scope = response.body["scopes"][1]
    assert repo_scope["escalation_recommendation"] == "escalate_stalled_scope"
    assert repo_scope["escalation_priority"] == "high"
    assert user_scope["escalation_recommendation"] == "no_escalation_needed"


def test_route_adapter_handles_memory_pressure_escalations(tmp_path: Path) -> None:
    database_path = tmp_path / "memory.sqlite"
    workspace = tmp_path / "workspace"
    workspace.mkdir()
    session_id = _seed_completed_session(database_path, workspace)
    ...
    response = RouteAdapter(create_app(database_path)).handle(
        RouteRequest(
            method="POST",
            path=f"/sessions/{session_id}/memory-escalations",
...
def _seed_completed_session(database_path: Path, workspace_root: Path) -> SessionId:
    bootstrap = SessionBootstrapService().build(
        SessionBootstrapCommand(
            title="Memory pressure escalations",
            user_input="Inspect escalations.",
