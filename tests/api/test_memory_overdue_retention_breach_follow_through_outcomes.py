...
def test_api_memory_overdue_retention_breach_follow_through_outcomes_classify_admin_override(
    tmp_path: Path,
) -> None:
    ...
    assert response.body["overdue_retention_breach_follow_through_outcome_counts"] == {
        "awaiting_admin_override_follow_through": 1
    }
    assert response.body["highest_priority_overdue_retention_breach_follow_through_outcome"] == (
    ...
    assert response.body["scopes"][0]["overdue_retention_breach_follow_through_outcome"] == (
        "awaiting_admin_override_follow_through"
    )


def test_route_adapter_handles_memory_overdue_retention_breach_follow_through_outcomes(
    tmp_path: Path,
) -> None:
    database_path = tmp_path / "memory.sqlite"
    workspace = tmp_path / "workspace"
    ...
    response = RouteAdapter(create_app(database_path)).handle(
        RouteRequest(
            method="POST",
            path=f"/sessions/{session_id}/memory-overdue-retention-breach-follow-through-outcomes",
...
def _seed_completed_session(database_path: Path, workspace_root: Path) -> SessionId:
    bootstrap = SessionBootstrapService().build(
        SessionBootstrapCommand(
            title="Memory overdue retention breach follow-through outcomes",
            user_input="Inspect overdue retention breach follow-through outcomes.",
