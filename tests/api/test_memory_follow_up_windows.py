...
def test_api_memory_follow_up_windows_pick_immediate_stalled_scope_follow_up(
    tmp_path: Path,
) -> None:
    ...
    stale_pending = _memory_record(
        memory_id="00000000-0000-0000-0000-000000000611",
    ...
    reviewed_user = _memory_record(
        memory_id="00000000-0000-0000-0000-000000000612",
        session_id=session_id,
    ...
    response = create_app(database_path).get_memory_escalation_follow_up_windows(
        str(session_id),
        {"user_id": "user-1", "as_of": "2026-07-09T00:00:00+00:00"},
    )

    assert response.status_code == 200
    assert response.body["follow_up_window_counts"] == {
        "immediate_follow_up": 1,
        "next_7d_review_window": 1,
    }
    assert response.body["highest_priority_follow_up_window"] == "immediate_follow_up"
    assert response.body["highest_priority_follow_up_priority"] == "high"
    assert response.body["highest_priority_follow_up_scope_kind"] == "repo"
    assert response.body["highest_priority_follow_up_due_at"] == "2026-07-09T00:00:00+00:00"
    assert response.body["highest_priority_follow_up_target_memory_id"] == str(
        stale_pending.memory_id
    )
    repo_scope = response.body["scopes"][0]
    user_scope = response.body["scopes"][1]
    assert repo_scope["follow_up_window"] == "immediate_follow_up"
    assert user_scope["follow_up_window"] == "next_7d_review_window"


def test_route_adapter_handles_memory_follow_up_windows(tmp_path: Path) -> None:
    database_path = tmp_path / "memory.sqlite"
    workspace = tmp_path / "workspace"
    workspace.mkdir()
    session_id = _seed_completed_session(database_path, workspace)
    ...
    response = RouteAdapter(create_app(database_path)).handle(
        RouteRequest(
            method="POST",
            path=f"/sessions/{session_id}/memory-follow-up-windows",
...
def _seed_completed_session(database_path: Path, workspace_root: Path) -> SessionId:
    bootstrap = SessionBootstrapService().build(
        SessionBootstrapCommand(
            title="Memory follow up windows",
