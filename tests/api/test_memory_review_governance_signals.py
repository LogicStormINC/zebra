...
def test_api_memory_review_governance_signals_report_backlog_and_reviews(
    tmp_path: Path,
) -> None:
    ...
    candidate = _memory_record(
        memory_id="00000000-0000-0000-0000-000000000291",
    ...
    reviewed = _memory_record(
        memory_id="00000000-0000-0000-0000-000000000292",
        session_id=session_id,
        visibility=MemoryVisibility.USER,
        memory_type=MemoryType.PREFERENCE,
        text="User reviewed memory.",
        user_id="user-1",
    ...
    _append_review_event(
        ...
        memory_type=reviewed.memory_type.value,
    ...
    response = create_app(database_path).get_memory_review_governance_signals(
        str(session_id),
        {"user_id": "user-1"},
    )
    ...
    assert response.body["total_reviewed_count"] == 1
    assert response.body["review_status_totals"] == {"confirmed": 1}
    repo_scope = response.body["scopes"][0]
    user_scope = response.body["scopes"][1]
    assert repo_scope["pending_by_type"] == {"procedure": 1}
    assert user_scope["reviewed_count"] == 1
    assert user_scope["latest_reviewed_at"] == "2026-07-08T10:30:00+00:00"


def test_route_adapter_handles_memory_review_governance_signals(tmp_path: Path) -> None:
    database_path = tmp_path / "memory.sqlite"
    workspace = tmp_path / "workspace"
    workspace.mkdir()
    session_id = _seed_completed_session(database_path, workspace)
    ...
    response = RouteAdapter(create_app(database_path)).handle(
        RouteRequest(
            method="POST",
            path=f"/sessions/{session_id}/memory-governance",
...
def _seed_completed_session(database_path: Path, workspace_root: Path) -> SessionId:
    bootstrap = SessionBootstrapService().build(
        SessionBootstrapCommand(
            title="Memory governance signals",
