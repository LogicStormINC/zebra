...
def test_api_memory_overdue_resolution_checkpoints_classify_next_review_confirmation(
    tmp_path: Path,
) -> None:
    ...
    SQLiteMemoryStore(database_path).upsert(
        _memory_record(
            memory_id="00000000-0000-0000-0000-000000001021",
    ...
    response = create_app(database_path).get_memory_overdue_resolution_checkpoints(
        str(session_id),
        {"as_of": "2026-07-11T06:00:00+00:00"},
    )

    assert response.status_code == 200
    assert response.body["overdue_resolution_checkpoint_counts"] == {
        "next_review_confirmation_checkpoint": 1
    }
    assert response.body["highest_priority_overdue_resolution_checkpoint"] == (
        "next_review_confirmation_checkpoint"
    )
    assert response.body["highest_priority_overdue_resolution_priority"] == "low"
    assert response.body["highest_priority_overdue_resolution_scope_kind"] == "repo"
    assert response.body["scopes"][0]["overdue_resolution_checkpoint"] == (
        "next_review_confirmation_checkpoint"
    )


def test_route_adapter_handles_memory_overdue_resolution_checkpoints(
    tmp_path: Path,
) -> None:
    database_path = tmp_path / "memory.sqlite"
    workspace = tmp_path / "workspace"
    ...
    response = RouteAdapter(create_app(database_path)).handle(
        RouteRequest(
            method="POST",
            path=f"/sessions/{session_id}/memory-overdue-resolution-checkpoints",
...
def _seed_completed_session(database_path: Path, workspace_root: Path) -> SessionId:
    bootstrap = SessionBootstrapService().build(
        SessionBootstrapCommand(
            title="Memory overdue resolution checkpoints",
            user_input="Inspect overdue resolution checkpoints.",
