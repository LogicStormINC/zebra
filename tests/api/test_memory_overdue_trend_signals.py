...
def test_api_memory_overdue_trend_signals_classify_emerging_overdue_scope(
    tmp_path: Path,
) -> None:
    ...
    SQLiteMemoryStore(database_path).upsert(
        _memory_record(
            memory_id="00000000-0000-0000-0000-000000000901",
    ...
    response = create_app(database_path).get_memory_overdue_trend_signals(
        str(session_id),
        {"as_of": "2026-07-10T00:00:00+00:00"},
    )

    assert response.status_code == 200
    assert response.body["overdue_trend_signal_counts"] == {"emerging_overdue": 1}
    assert response.body["highest_priority_overdue_trend_signal"] == "emerging_overdue"
    assert response.body["highest_priority_overdue_trend_rank"] == 1
    assert response.body["highest_priority_overdue_trend_scope_kind"] == "repo"
    assert response.body["scopes"][0]["overdue_trend_signal"] == "emerging_overdue"


def test_route_adapter_handles_memory_overdue_trends(tmp_path: Path) -> None:
    database_path = tmp_path / "memory.sqlite"
    workspace = tmp_path / "workspace"
    workspace.mkdir()
    session_id = _seed_completed_session(database_path, workspace)
    ...
    response = RouteAdapter(create_app(database_path)).handle(
        RouteRequest(
            method="POST",
            path=f"/sessions/{session_id}/memory-overdue-trends",
            body={"tenant_id": "tenant-1", "as_of": "2026-07-10T00:00:00+00:00"},
        )
    )
...
def _seed_completed_session(database_path: Path, workspace_root: Path) -> SessionId:
    bootstrap = SessionBootstrapService().build(
        SessionBootstrapCommand(
            title="Memory overdue trend signals",
            user_input="Inspect overdue trends.",
