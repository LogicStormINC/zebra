...
def test_api_memory_pressure_action_hints_pick_high_priority_oldest_review_action(
    tmp_path: Path,
) -> None:
    ...
    stale_pending = _memory_record(
        memory_id="00000000-0000-0000-0000-000000000411",
    ...
    reviewed_user = _memory_record(
        memory_id="00000000-0000-0000-0000-000000000412",
        session_id=session_id,
    ...
    response = create_app(database_path).get_memory_pressure_action_hints(
        str(session_id),
        {"user_id": "user-1", "as_of": "2026-07-09T00:00:00+00:00"},
    )

    assert response.status_code == 200
    assert response.body["action_hint_counts"] == {
        "review_oldest_pending": 1,
        "no_action_needed": 1,
    }
    assert response.body["highest_priority_action_hint"] == "review_oldest_pending"
    assert response.body["highest_priority_action_priority"] == "high"
    assert response.body["highest_priority_action_scope_kind"] == "repo"
    assert response.body["highest_priority_action_target_memory_id"] == str(
        stale_pending.memory_id
    )
    repo_scope = response.body["scopes"][0]
    user_scope = response.body["scopes"][1]
    assert repo_scope["action_hint"] == "review_oldest_pending"
    assert repo_scope["action_priority"] == "high"
    assert user_scope["action_hint"] == "no_action_needed"


def test_route_adapter_handles_memory_pressure_action_hints(tmp_path: Path) -> None:
    database_path = tmp_path / "memory.sqlite"
    workspace = tmp_path / "workspace"
    workspace.mkdir()
    session_id = _seed_completed_session(database_path, workspace)
    ...
    response = RouteAdapter(create_app(database_path)).handle(
        RouteRequest(
            method="POST",
            path=f"/sessions/{session_id}/memory-action-hints",
...
def _seed_completed_session(database_path: Path, workspace_root: Path) -> SessionId:
    bootstrap = SessionBootstrapService().build(
        SessionBootstrapCommand(
            title="Memory pressure action hints",
            user_input="Inspect action hints.",
