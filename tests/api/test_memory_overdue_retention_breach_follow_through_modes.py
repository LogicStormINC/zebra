...
def test_api_memory_overdue_retention_breach_follow_through_modes_classify_admin_override(
    tmp_path: Path,
) -> None:
    ...
    assert response.body["overdue_retention_breach_follow_through_counts"] == {
        "admin_override_follow_through": 1
    }
    assert response.body["highest_priority_overdue_retention_breach_follow_through_mode"] == (
    ...
    assert (
        response.body["highest_priority_overdue_retention_breach_follow_through_scope_kind"]
        == "repo"
    )
    assert response.body["scopes"][0]["overdue_retention_breach_follow_through_mode"] == (
        "admin_override_follow_through"
    )


def test_route_adapter_handles_memory_overdue_retention_breach_follow_through_modes(
    tmp_path: Path,
) -> None:
    database_path = tmp_path / "memory.sqlite"
    workspace = tmp_path / "workspace"
    ...
    response = RouteAdapter(create_app(database_path)).handle(
        RouteRequest(
            method="POST",
            path=f"/sessions/{session_id}/memory-overdue-retention-breach-follow-through-modes",
...
def _seed_completed_session(database_path: Path, workspace_root: Path) -> SessionId:
    bootstrap = SessionBootstrapService().build(
        SessionBootstrapCommand(
            title="Memory overdue retention breach follow-through modes",
            user_input="Inspect overdue retention breach follow-through modes.",
