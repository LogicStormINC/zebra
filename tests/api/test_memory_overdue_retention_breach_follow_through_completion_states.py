...
def test_api_follow_through_completion_states_classify_admin_override(
    tmp_path: Path,
) -> None:
    ...
    assert response.body["overdue_retention_breach_follow_through_completion_counts"] == {
        "admin_override_completion_pending": 1
    }
    assert (
        response.body[
...
def test_route_adapter_handles_follow_through_completion_states(
    tmp_path: Path,
) -> None:
    database_path = tmp_path / "memory.sqlite"
    workspace = tmp_path / "workspace"
...
def _seed_completed_session(database_path: Path, workspace_root: Path) -> SessionId:
    bootstrap = SessionBootstrapService().build(
        SessionBootstrapCommand(
            title="Memory overdue retention breach follow-through completion states",
