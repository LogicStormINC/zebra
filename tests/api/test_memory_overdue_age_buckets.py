...
def test_api_memory_overdue_age_buckets_classify_repo_and_user_scopes(
    tmp_path: Path,
) -> None:
    ...
    stale_pending = _memory_record(
        memory_id="00000000-0000-0000-0000-000000000811",
    ...
    reviewed_user = _memory_record(
        memory_id="00000000-0000-0000-0000-000000000812",
        session_id=session_id,
    ...
    response = create_app(database_path).get_memory_overdue_age_buckets(
        str(session_id),
        {"user_id": "user-1", "as_of": "2026-07-09T00:00:00+00:00"},
    )

    assert response.status_code == 200
    assert response.body["overdue_age_bucket_counts"] == {
        "lt_1d_overdue": 1,
        "not_overdue": 1,
    }
    assert response.body["highest_priority_overdue_age_bucket"] == "lt_1d_overdue"
    assert response.body["highest_priority_overdue_age_scope_kind"] == "repo"
    assert response.body["highest_priority_overdue_age_days"] == 0
    assert response.body["highest_priority_overdue_age_target_memory_id"] == str(
        stale_pending.memory_id
    )
    repo_scope = response.body["scopes"][0]
    user_scope = response.body["scopes"][1]
    assert repo_scope["overdue_age_bucket"] == "lt_1d_overdue"
    assert repo_scope["overdue_age_days"] == 0
    assert user_scope["overdue_age_bucket"] == "not_overdue"


def test_route_adapter_handles_memory_overdue_age_buckets(tmp_path: Path) -> None:
    database_path = tmp_path / "memory.sqlite"
    workspace = tmp_path / "workspace"
    workspace.mkdir()
    session_id = _seed_completed_session(database_path, workspace)
    ...
    response = RouteAdapter(create_app(database_path)).handle(
        RouteRequest(
            method="POST",
            path=f"/sessions/{session_id}/memory-overdue-age-buckets",
...
def _seed_completed_session(database_path: Path, workspace_root: Path) -> SessionId:
    bootstrap = SessionBootstrapService().build(
        SessionBootstrapCommand(
            title="Memory overdue age buckets",
            user_input="Inspect overdue age buckets.",
