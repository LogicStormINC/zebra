...
def test_memory_pressure_action_hints_contract_matrix_matches_across_api_and_cli(
    tmp_path: Path,
) -> None:
    ...
    api_pending = _memory_record(
        memory_id="00000000-0000-0000-0000-000000000431",
    ...
    cli_pending = _memory_record(
        memory_id="00000000-0000-0000-0000-000000000432",
    ...
    http_response = TestClient(create_http_app(api_database_path)).post(
        f"/sessions/{api_session_id}/memory-action-hints",
...
def _normalize_http(payload: dict[str, object]) -> dict[str, object]:
    return {
        ...
        "action_hint_counts": payload["action_hint_counts"],
        "highest_priority_action_hint": payload["highest_priority_action_hint"],
        "highest_priority_action_priority": payload["highest_priority_action_priority"],
        "highest_priority_action_scope_kind": payload["highest_priority_action_scope_kind"],
    }
...
def _seed_completed_session(database_path: Path, workspace_root: Path) -> SessionId:
    bootstrap = SessionBootstrapService().build(
        SessionBootstrapCommand(
            title="Memory action hints matrix",
